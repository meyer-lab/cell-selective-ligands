...
All analysis was implemented in Julia, and can be found at <https://github.com/meyer-lab/cell-selective-ligands>.

### Generalized multi-ligand, multi-receptor multivalent binding model

To model the binding consequences of multivalent mixtures, we extended our previous binding model to account for complexes with mixed ligand composition [@pmid:29960887].

Assume we have $N_L$ types of monomer ligands and $N_R$ type of receptors. The association constant of the initial monovalent binding between ligand $i$ and receptor $j$ is $K_{a,ij}$. All additional binding after the initial interaction between ligand $i$ and receptor $j$ is assumed to have an association constant $K_x^* K_{a,ij}$, proportional to the pair's corresponding monovalent binding constant. The concentration of complexes is $L_0$, and the complexes consist of various ligand monomers. The proportion of ligand $i$ in all monomers is $C_i$. By this setup, we know that $\sum_{i=1}^{N_L} C_i = 1$. Let $R_{\mathrm{tot},i}$ be the total number of receptor $i$ expressed on the cell surface, and $R_{\mathrm{eq},i}$ be the number of unbound receptor $i$ on that cell at the equilibrium state during the ligand complex-receptor interaction.
...
When complexes are precisely engineered and not randomly assorted, such as with bispecific antibodies, the proportions of each kind of complexes become exogenous variables and are no longer decided by the monomer composition $C_i$'s. The monomer composition of a ligand complex can be represented by a vector $\mathbf{θ} = (θ_1, θ_2, ..., θ_{N_L})$, where each $θ_i$ is the number of monomer ligand type $i$ on that complex. Let $C_{\mathbf{θ}}$ be the proportion of the $\mathbf{θ}$ complexes in all ligand complexes, and $Θ$ be the set of all possible $\mathbf{θ}$'s. We have $\sum_{\mathbf{θ} \in \Theta} C_{\mathbf{θ}} = 1$.
