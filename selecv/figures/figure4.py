...
def makeFigure():
    ...
    heatmapDL(ax[2], np.exp(optParams[0]), np.exp(optParams[1]), np.array([[DLaffs[0], DLaffs[1]], [np.exp(optParams[4]), np.exp(optParams[5])]]),
              [0.5, 0.5], Cplx=np.array([[fDLsub, 0], [0, optParams[2]]]), vrange=(-8, 4), cbar=True, highlight=[r"$R_1^{med}R_2^{lo}$"], lowlight=[r"$R_1^{hi}R_2^{lo}$"])
    heatmapDL(ax[3], np.exp(optParams[0]), np.exp(optParams[1]), np.array([[DLaffs[0], DLaffs[1]], [np.exp(optParams[4]), np.exp(optParams[5])]]),
              [0.5, 0.5], Cplx=np.array([[fDLsub, 0], [0, optParams[2]]]), vrange=(2, 16), cbar=False, dead=True, highlight=[r"$R_1^{med}R_2^{lo}$"], lowlight=[r"$R_1^{hi}R_2^{lo}$"])
    heatmapDL(ax[4], np.exp(optParams[0]) / 2, np.exp(optParams[1]), np.array([[DLaffs[0], DLaffs[1]], [np.exp(optParams[4]), np.exp(optParams[5])]]),
              [0, 1], Cplx=np.array([[fDLsub, 0], [0, optParams[2]]]), vrange=(2, 16), cbar=True, dead=False, jTherap=True, highlight=[r"$R_1^{med}R_2^{lo}$"], lowlight=[r"$R_1^{hi}R_2^{lo}$"])
...
def mixScanOpt(ax, targetPop, specPops=False):
    ...
    for ii, mix in enumerate(mixs):
        ...
        AffDF = AffDF.append(pd.DataFrame({"Agonist Mix": [mix / 1e-9], "Receptor": r"$R_{1}$", "Affinity": optimArr[0], "Ligand": "Agonist"}))
        AffDF = AffDF.append(pd.DataFrame({"Agonist Mix": [mix / 1e-9], "Receptor": r"$R_{2}$", "Affinity": optimArr[1], "Ligand": "Agonist"}))
