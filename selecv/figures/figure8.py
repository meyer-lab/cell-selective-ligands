...
def makeFigure(KxStar = 1e-12):
    ...
    competeHeatmap(ax[0], L0, KxStar, [[1, 1], [1, 0]], Comp, Kav, title = "[1, 0]")
    competeHeatmap(ax[1], L0, KxStar, [[1, 1], [0, 1]], Comp, Kav, title = "[0, 1]")
    competeHeatmap(ax[2], L0, KxStar, [[1, 1], [2, 0]], Comp, Kav, title = "[2, 0]")
    competeHeatmap(ax[3], L0, KxStar, [[1, 1], [0, 2]], Comp, Kav, title = "[0, 2]")
    competeHeatmap(ax[4], L0, KxStar, [[1, 1], [2, 1]], Comp, Kav, title = "[2, 1]")
    competeHeatmap(ax[5], L0, KxStar, [[1, 1], [1, 2]], Comp, Kav, title = "[1, 2]")

    f.suptitle("$K_x^* = ${}".format(KxStar))
    return f


def Cplx1to2Ratio(L0, KxStar, Rtot, Cplx, Ctheta, Kav):
    """ Always LigCplx 1 / LigCplx 0 """
    Lbound, Rbound = polyc(L0, KxStar, Rtot, Cplx, Ctheta, Kav)
    return Lbound[1] / Lbound[0]


def competeHeatmap(ax, L0, KxStar, Cplx, Comp, Kav, vrange=(-3, 3), cbar=True, layover=False, title = ""):
    abundRange = 2, 6
    nAbdPts = 70
    abundScan = np.logspace(abundRange[0], abundRange[1], nAbdPts)

    func = np.vectorize(lambda abund1, abund2: Cplx1to2Ratio(L0, KxStar, [abund1, abund2], Cplx, Comp, Kav))
