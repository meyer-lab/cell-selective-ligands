...
def polyfc(Lmono, KxStar, f, Rtot, LigC, Kav):
    ...
    Req = Req_Regression(Lmono, KxStar, f, Rtot, LigC, Kav)
...
def Req_Regression(Lmono, KxStar, f, Rtot, LigC, Kav):
    ...
    x0 = np.max(LmonoA, axis=0)
    x0 = np.multiply(1.0 - np.divide(x0, 1 + x0), Rtot)

    # Solve Req by calling least_squares() and Req_func()
    lsq = root(Req_func, x0, method="lm", args=(Rtot, LmonoA, AKxStar, f), options={"maxiter": 3000})
